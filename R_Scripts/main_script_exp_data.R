...
#Plotting AF for all the samples
af_ho<- plot_af_gen(data_ho%>%filter(gen %in% c("10")), "OregonR")
af_hs<- plot_af_gen(data_hs%>%filter(gen %in% c("10")), "Helsinki")+ theme(axis.title.x = element_blank())
af_so<- plot_af_gen(data_so%>%filter(gen %in% c("10")), "Samarkand")+ theme(axis.title.x = element_blank())
af_comp<- af_so/af_hs/af_ho+plot_layout(guides = "collect", axis_titles= "collect_x") &
  theme(legend.position = "bottom")&plot_annotation(tag_levels = list(c("A", "B", "C")))
af_comp <- af_comp &
  guides(
    color = guide_legend(title = "Samples"),
    fill  = guide_legend(title = "Samples"),
...
sel_hel_hs<- sel_hs%>%dplyr::select(-sam_s_hs)%>%dplyr::rename("s"=hel_s_hs)%>%
  mutate(group="HS-SH")
merge_hs_10<- compute_p(model_df_hs_10, sel_hel_hs, "HO-OH")
...
sel_ore_ho<- sel_ho%>%dplyr::select(-hel_s_ho)%>%dplyr::rename("s"=ore_s_ho)
merge_ho_10<- compute_p(model_df_ho_10, sel_ore_ho, "SO-OS")
...
sel_sam_so<- sel_so%>%dplyr::select(-ore_s_so)%>%dplyr::rename("s"=sam_s_so)
merge_so_10<- compute_p(model_df_so_10, sel_sam_so, "HS-SH")
...
plot_class_res_10<-plot_class(merge_dev_10, merge_df_10, "1", "10")
print(plot_class_res_10)
